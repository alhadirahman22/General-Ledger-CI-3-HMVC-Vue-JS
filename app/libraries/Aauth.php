...
class Aauth
{
    ...
    public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL)
    {
        ...
        if ($query->num_rows() != 0 && $this->verify_password($password, $row->pass)) {
            ...
            // get level user
            $data_level = $this->CI->db->select('a.definition')
                ->join('aauth_user_to_group as b', 'b.group_id = a.id', 'join')
                ->where('b.user_id', $row->id)
                ->limit(1)
                ->get('aauth_groups as a');

            if ($data_level) {
                $data['level_user'] = $data_level->row()->definition;
            }
    ...
    public function redirect_page($identifier, $pass, $remember = FALSE, $totp_code = NULL)
    {
        // Remove cookies first
        $cookie = array(
            'name' => 'user',
        ...
        if ($query->num_rows() != 0 && $this->verify_password($password, $row->pass)) {
            ...
            // get level user
            $data_level = $this->CI->db->select('a.*')
                ->join('aauth_user_to_group as b', 'b.group_id = a.id', 'join')
                ->where('b.user_id', $row->id)
                ->limit(1)
                ->get('aauth_groups as a');
            ...
            $result = array('status' => TRUE, 'level' => $data_level->row());
            return $result;
            // return $data;
        }
        // if not matches
    ...
    public function create_user($email = '', $pass = '', $username = FALSE, $group = FALSE, $employee_id = '', $user_id)
    {
     ...
     * @return bool Update fails/succeeds
     */
    public function update_user($user_id = '', $email = FALSE, $pass = FALSE, $username = FALSE, $group = FALSE, $employee_id = '', $updated_by)
    {
    ...
    public function delete_user($user_id)
    {
        ...
        // delete user vars
        if ($this->config_vars['user_variables']) {
            $this->aauth_db->where('user_id', $user_id);
    ...
    public function create_group($group_name, $definition = '', $dashboard = '', $created_by)
    {
    ...
    public function create_perm($perm_name, $definition = '', $help_uri = '', $is_crud, $created_by)
    {
        ...
        if ($query->num_rows() < 1) {
            ...
            if ($is_crud == '1') {
                $arr_crud = ['add', 'delete', 'edit'];
                for ($i = 0; $i < count($arr_crud); $i++) {

                    $data = array(
                        'name' => $perm_name . '/' . $arr_crud[$i],
                        'definition' => $definition . ' - ' . ucwords($arr_crud[$i]),
                        'created_at' => Date('Y-m-d H:i:s'),
                        'created_by' => $created_by
                    );
                    $this->aauth_db->insert($this->config_vars['perms'], $data);
     ...
     * @return bool Update success/failure
     */
    public function update_perm($perm_par, $perm_name = FALSE, $definition = FALSE, $help_uri = '', $updated_at)
    {

        $perm_id = $this->get_perm_id($perm_par);

        $data['help_uri'] = $help_uri;
    ...
    public function is_allowed($perm_par, $user_id = FALSE)
    {
        ...
        if (empty($perm_par)) { // handling perm_par empty, while query SELECT * FROM `aauth_perms` WHERE `name` = 0, it is detected having data value {confusing}
            return false;
        }
