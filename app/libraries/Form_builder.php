...
class Form_builder
{
    ...
    private $config = array(/* Config array - can be overrided by passing in array in ini() */
        ...
        'default_datetime_class' => 'form-datetime',
    ...
    function build_form_horizontal($options, $data_source = array())
    {
        ...
        foreach ($options as $elm_options) {
            ...
            if (is_array($this->elm_options)) {
                /* overwrite label to flag is a required or not
                    for form tab
                   custom by alhadi rahman 2021-09-18

                */
                if (isset($this->elm_options['required'])) {
                    $this->elm_options['required2'] = $this->elm_options['required'];
                    unset($this->elm_options['required']);
                }
                $this->_prep_options();
                switch ($this->func) {
                    case 'form_hidden':
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_time':
                    ...
                    break;
                case 'form_datetime':
                    // $this->elm_options['class'] = $this->config['default_dropdown_class'] . ' ' . $this->config['default_datetime_class'];
                    // $this->elm_options['type'] = 'text';
                    // $input_html_string = form_datetime($this->elm_options);
                    $this->elm_options['value'] = ($this->elm_options['value'] != '') ? date('Y-m-d\TH:i:s', strtotime($this->elm_options['value'])) : '';
                    $this->elm_options['type'] = 'datetime-local';
                    $input_html_string = form_input($this->elm_options);
                    break;
                case 'form_hidden':
                ...
                case 'form_multiselect':
                    ...
                    $options = (isset($this->elm_options['options'])) ? $this->elm_options['options'] : [];
                ...
                case 'form_image':
                    ...
                    if ($value) {
                        ...
                        $img_path = (isset($this->elm_options['image_path'])) ? $this->elm_options['image_path'] : '';
                        $file_preview = '<img src="' . $img_path . $value . '">';
                    } else {
                    ...
                    $input_html_string = '<div class="fileinput ' . $file_input_class . '" data-provides="fileinput">
                                ...
                                <div>
                                    <span class="btn btn-sm btn-raised btn-default btn-file">
                                        <span class="fileinput-new"><i class="icon-upload"></i> ' . lang('select_image') . '</span>
                                        <span class="fileinput-exists"><i class="icon-upload"></i> ' . lang('change') . '</span>
                                        <input type="file" name="' . $this->elm_options['name'] . '" accept="image/*">
                                    </span>
                                    <input type="hidden" name="delete_' . $this->elm_options['name'] . '" value="">
                                    <a href="#" class="btn btn-sm btn-raised btn-danger fileinput-exists delete_' . $this->elm_options['name'] . '" data-dismiss="fileinput"><i class="icon-trash"></i> ' . lang('delete') . '</a>
                                </div>
                            </div>';
                    break;
                default:
    ...
    private function _label($link_to_input_id = TRUE)
    {
        ...
        /* overwrite label to flag is a required or not
           custom by alhadi rahman 2021-08-02

        */

        $el =  $this->elm_options;
        if (isset($el['required2']) && strtoupper($el['required2']) == 'TRUE') {
