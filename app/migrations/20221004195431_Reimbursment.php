...
class Migration_Reimbursment extends CI_Migration
{
    public function up()
    {
        ...
        Capsule::schema()->create('reimbursment_approval_log', function ($table) {
            ...
            $table->bigInteger('parent_reimbursment_approval_log_id')->nullable(true);
            $table->bigInteger('reimbursment_dept_approval_emp_id');
            $table->text('desc')->nullable(true);
            $table->enum('status', ['1', '-1', '0', '2', '-2'])->comment('1 = approved, 0 = not action, -1 = reject, 2 =  progress, -2 = awaiting, 99 = Terbayarkan ')->default('0');
            $table->integer('created_by')->nullable(true);
            $table->integer('updated_by')->nullable(true);
            $table->timestamps();
        });
