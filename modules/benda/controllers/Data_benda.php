...
class Data_benda extends CI_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->perm = 'benda/data_benda';
        ...
        $this->data['table'] = [
            'columns' => [
                ...
                '8' => ['name' => 'a.tags', 'title' => 'Tag ', 'filter' => ['type' => 'text'], 'class' => 'default-sort', 'sort' => 'asc'],
                '9' => ['name' => 'i.name', 'title' => 'Jenis Mutasi ', 'filter' => ['type' => 'text'], 'class' => 'default-sort', 'sort' => 'asc'],
                '10' => ['name' => 'h.status', 'title' => 'Status Mutasi ', 'filter' => ['type' => 'dropdown', 'options' => ['' => 'All', '0' => 'Waiting', '1' => 'Approved', '-1' => 'Reject', '2' => 'Progress']], 'class' => 'default-sort', 'sort' => 'asc'],
        ...
        if ($this->aauth->is_allowed($this->perm . '/edit') || ($this->aauth->is_allowed($this->perm . '/delete'))) {
            $this->data['table']['columns']['11'] = ['name' => 'id', 'title' => '', 'class' => 'no-sort text-center', 'width' => '5%', 'filter' => ['type' => 'action']];
        }
    ...
    public function index()
    {
        ...
        if (count($data_museum) > 1) {
            $this->data['btn_option'] = $this->aauth->is_allowed($this->perm . '/add')
                ? '<a href="javascript:void(0);" class="btn btn-sm btn-primary show-list-museum"><i class="ace-icon fa fa fa-plus-circle bigger-110"></i> Registrasi / Heregistrasi</a>' : '';
        } else {
            $this->data['btn_option'] = $this->aauth->is_allowed($this->perm . '/add')
                ? '<a href="' . $this->data['module_url'] . 'form/' . $data_museum[0]['encry'] . '" class="btn btn-sm btn-primary"><i class="ace-icon fa fa fa-plus-circle bigger-110"></i>Registrasi / Heregistrasi</a>' : '';
        }
    ...
    public function get_list()
    {
        ...
        if ($datas) {
            foreach ($datas->result_array() as $data) {
                ...
                $changed = $this->db->where('benda_id', $data[$this->table_id_key])->from('benda_histories')->count_all_results();

                $view_changed = ($changed > 0) ? '<div><a href="javascript:void(0)" data-id="' . $data[$this->table_id_key] . '" class="viewsunting" style="font-size: 10px;">' . $changed . 'x disunting</a></div>' : '';

                $data_status = ['' => '', '0' => 'Waiting', '1' => 'Approved', '-1' => 'Reject', '2' => 'Progress'];

                $output['data'][] = array(
                    '<div style="text-align:center;">' . $photo_profile . '</div>',
                    '<b>' . $data['name'] . '</b>' . $view_changed,
                    $data['alias'],
    ...
    public function form($token = '')
    {
        ...
        $form = [
            ...
            array(
                ...
                'label' => '',
                'form_control_class' => 'col-md-12',
                'html' => $this->_html_sparator('Cerita', '')
            ),


            array(
                'id' => 'history_id',
                'label' => 'Pilih Cerita Yang Berhubungan',
                'type' => 'dropdown',
                'form_control_class' => 'col-md-4',
                'class' => 'select2-serverside-filter-museum',
                'data-museumid' => $dataToken->museum_id,
                ...
                'data-id' => 'history_id',
                'data-text' => 'title',
                'data-selected' => ($data) ? $data->history_id : '',
            ),
            ...
            array(
                'id' => 'div_track_history',
                'type' => 'html',
                'label' => '',
                'form_control_class' => 'col-md-4',
                'html' => $this->_html_track_history($data)
            ),
        ...
        $this->data['benda_id'] = $benda_id;
        $this->data['museum_id'] = $dataToken->museum_id;
    ...
    public function save()
    {
        ...
        $data_tag_id = (isset($data['tag_id'])) ? array_filter($data['tag_id']) : [];

        $track_history = (isset($data['track_history'])) ? $data['track_history'] : '';
        ...
        if ($this->form_validation->run() === true) {
            ...
            do {
                if (!$data[$this->table_id_key]) {
                    ...
                    $data_trx = array(
                        'benda_id' => $benda_id,
                        'data_history' => $track_history,
                        'created_at' => Date('Y-m-d H:i:s'),
                        'created_by' => $this->data['user']->id
                    );

                    $this->db->insert('benda_histories', $data_trx);
                }
                ...
                $this->db->reset_query();
                if (count($data_tag_id) > 0) {
                    $tags_view = '';
                    for ($i = 0; $i < count($data_tag_id); $i++) {
                        $data_tags_name = $this->db->select('name')->get_where('tags', array('tag_id' => $data_tag_id[$i]))->row();
                        if ($data_tags_name) {
                            $koma = ($i != 0) ? ', ' : '';
                            $tags_view = $tags_view . $koma . $data_tags_name->name;

                            $data_ins = array(
                                'benda_id' => $benda_id,
                                'tag_id' => $data_tag_id[$i],
                                'created_at' => Date('Y-m-d H:i:s'),
                                'created_by' => $this->data['user']->id
                            );
                            $this->db->insert('benda_tags', $data_ins);
                            $this->db->reset_query();
                        }
                    }

                    $this->db->where($this->table_id_key, $data[$this->table_id_key]);
                    $this->db->update($this->table, array('tags' => $tags_view));
                    $this->db->reset_query();
                }
    ...
    public function form_changed($token)
    {
        $data = array();
        $dataToken = get_jwt_decryption($token);
        $id = $dataToken->id;
        ...
        $data = $this->m_master
            ->get('benda_histories', array('benda_history_id' => $id));

        if ($data) {
            // print_r(json_encode($data->data_history));
            // die();
            // $data->data_history = ;



            $this->data['created_by'] = $this->m_master->get_username_by($data->created_by);
            $this->data['data'] = $data;
            $script_add = '<script type="text/javascript">const dataModule = ' . json_encode($this->data) . '</script>';
            $this->data['script_add'] = $script_add;
            ...
            $this->template->form();
            $this->output->set_title(($this->data['data'] ? lang('edit') : lang('add')) . ' ' . lang('heading'));
            $this->load->view('form_changed', $this->data);
            $this->load->js('assets/custom/js/museum/form_changed.js');
        } else {
            show_404();
            exit();
        }
    ...
    public function _html_tags($data)
    {
        ...
        $opstions = '';
        if (isset($data->benda_id) && $data->benda_id != '') {
            $dataTags = $this->db->where('a.benda_id', $data->benda_id)
                ->join('tags AS b', 'b.tag_id = a.tag_id', 'left')
                ->select('a.tag_id AS id, b.name AS text')
                ->get('benda_tags AS a')->result_array();

            if (count($dataTags) > 0) {
                for ($i = 0; $i < count($dataTags); $i++) {
                    $total = $this->db
                        ->where('a.tag_id', $dataTags[$i]['id'])
                        ->where('b.active', 1)
                        ->join('bendas AS b', 'b.benda_id = a.benda_id', 'left')
                        ->from('benda_tags AS a')->count_all_results();

                    $vew_total = ($total > 0) ? ' (' . $total . ')' : '';
                    // $dataTags[$i]['text'] = $dataTags[$i]['text'] . $vew_total;
                    $dataTags[$i]['text'] = $dataTags[$i]['text'];
                    // $dataTags[$i]['title'] = $dataTags[$i]['text'];
                    $opstions = $opstions . '<option value="' . $dataTags[$i]['id'] . '" selected>' . $dataTags[$i]['text'] . '</option>';
                }
            }
        }
        ...
        $html = '<div class="row">
            <div class="col-sm-12">
                <select size="10" name="tag_id[]" id="tag_id" multiple="multiple">' . $opstions . '</select>
            </div>
        </div>';
        return $html;
    ...
    public function select2tags()
    {
        ...
        $data = $this->db
            ...
            ->where('a.active', 1)
            ->limit(15)
            ->select('a.tag_id AS id, a.name AS text')
            ->get('tags AS a')->result_array();

        $dataResult = array();
        if (count($data) > 0) {
            for ($i = 0; $i < count($data); $i++) {
                $total = $this->db
                    ->where('a.tag_id', $data[$i]['id'])
                    ->where('b.active', 1)
                    ->join('bendas AS b', 'b.benda_id = a.benda_id', 'left')
                    ->from('benda_tags AS a')->count_all_results();
    ...
    public function getHistory()
    {
        ...
        if ($data) {
            ...
            $data->encry = get_jwt_encryption($payload);
        }

        return print_r(json_encode($data));
    }

    public function _html_track_history($data)
    {
        $html = '<div><textarea class="hide" name="track_history" id="track_history"></textarea></div>';
        return $html;
    }

    public function getListSunting()
    {
        $benda_id = $_GET['benda_id'];
        $data = $this->db
            ->where('benda_id', $benda_id)
            ->order_by('created_at', 'DESC')
            ->get('benda_histories')->result_array();

        if (count($data) > 0) {
            for ($i = 0; $i < count($data); $i++) {
                $payload = array(
                    'id' => $data[$i]['benda_history_id'],
                );
                $encry = get_jwt_encryption($payload);

                $data[$i]['encrypt'] = $encry;
            }
        }
