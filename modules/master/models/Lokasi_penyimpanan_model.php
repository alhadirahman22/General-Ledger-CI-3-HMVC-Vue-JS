...
class Lokasi_penyimpanan_model extends CI_Model
{
    ...
    function get_all($start = 0, $length = 10, $filter = array(), $order = array())
    {
        ...
        $this->db->select('a.*, b.name AS museum_name, c.name AS parent_name');
    ...
    function filter($filter = array())
    {
        $this->db
            ...
            ->join('lokasi_penyimpanan AS c', 'c.parent_id = a.lokasi_penyimpanan_id', 'left');
